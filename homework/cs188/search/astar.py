...
"""
Astar uses both path_cost and heuristic to guide search direction

f(n) = g(n) + h(n)

g(n) is path_cost_so_far
...
class Problem:
    ...
    def expand(self, A):
        return [Node(state = next_state,
                     parent = A,
                     path_cost=(A.path_cost + self.graph.get(A.state, next_state) or infinity),
                     heuristic = distance(romania.locations.get(next_state),
                                          romania.locations.get(self.goal.state)))
...
class PriorityQueue:
    ...
    def append(self, items):
        for item in items:
            print("--> adding", item, item.heuristic)
            bisect.insort(self.A, (item.heuristic + item.path_cost, item))
...
NodeA = Node('A', heuristic=distance(romania.locations.get('A'),romania.locations.get('B')))
NodeB = Node('B', heuristic=0)

problem = Problem(initial=NodeA, goal=NodeB, graph=romania)
