...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if d < 0.0:
            raise ValueError, "moving backward is not acceptable"            
        ...
        turn = d2/self.length * tan(steering2)

        if abs(turn) < 0.001:
            new_x = self.x + d2*cos(self.orientation)
            new_y = self.y + d2*sin(self.orientation)
            ...
            new_orientation = (self.orientation + turn) % (2.0 * pi)
            new_x = cx + sin(new_orientation)*R
            new_y = cy - cos(new_orientation)*R
    ...
    def sense(self, flag = 1): #do not change the name of this function
        ...
        for T in landmarks:
            dy = T[0] - self.y
            dx = T[1] - self.x
            z = atan2(dy, dx) - self.orientation;
            z = z % (2.0 * pi)
            if flag == 1:
                z = random.gauss(z, self.bearing_noise)
            Z.append(z)
