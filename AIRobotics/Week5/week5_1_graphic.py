    ...
    while change >= tolerance:
        ...
        for i in range(len(path)):
            for j in range(len(path[0])):
                ...
                i2 = i-1
                i3 = (i+1)%len(path)
                newpath[i][j] += weight_data * (path[i][j] - newpath[i][j]) + weight_smooth * (newpath[i2][j] + newpath[i3][j] - 2.0 * newpath[i][j])                
    ...
    for i in range(len(newpath)): 
        for j in range(len(newpath[0])):
            if not close_enough(newpath[i][j], answer[i][j]):
                print 'Error, at least one of your entries is not correct.', i, j, newpath[i][j], answer[i][j]
                return False
...
for i in range(len(testpath1)):
    x.append(testpath1[i][0])
    y.append(testpath1[i][1])
    x1.append(newpath[i][0])
    y1.append(newpath[i][1])
